...
##############################################################
# THESE WORK IN ARMATURE EDIT MODE
##############################################################

def translate_ebone_local(name, vector):
...
def bone_radius_check_points(name, number, dist):
    ebone = bpy.context.object.data.edit_bones[name]
    ...
    for n in range(number):
        v = Vector((0, 0, dist))
        rot_mat = Matrix.Rotation(radians((360 / number) * n), 4, 'Y') @ mat
        v = v @ rot_mat
...
def rotate_ebone_local(name, angle, axis):
    ebone = bpy.context.object.data.edit_bones[name]
...
def subdivide_ebone(name, number, number_to_keep, reverse_naming, prefix, parent_all_to_source, delete_source):
        ...
        def main():
            ...
            def create_no_twist_bone(source_bone):
                ...
                if rig.data.bones.get(no_twist_name) == None:
                    duplicate_bone(source_name=source_bone, new_name=no_twist_name, parent_name='SOURCE_PARENT',
                                   roll_override=False, length_override='HALF')
                ...
                c.head_tail = 1
                bone_settings(name=no_twist_name, layer=misc_layer, group=None, deform=False, lock_loc=True,
                              lock_rot=False, lock_scale=True, type=None)
                return no_twist_name

            def create_twist_bones(source_bone, count, upper_or_lower_limb, twist_target_distance, twist_target_parent,
                                   lock_xz_rot, end_affector, source_bone_bend_axis):
                ...
                ebones = rig.data.edit_bones
                source_ebone = ebones[source_bone]

                rev = True if upper_or_lower_limb == 'lower' else False
                twist_bones = subdivide_ebone (name=source_bone, number=3, number_to_keep=count,
                                               reverse_naming=rev, prefix='twist', parent_all_to_source=True,
                                               delete_source=False)
                ...
                if upper_or_lower_limb == 'upper':
                    ...
                    translate_ebone_local(name=twist_target_name, vector=vector)
                    ebone = ebones[twist_target_name]
                    ebone.tail = ebone.head + Vector((0, 0, general_bone_size))
                    bone_settings(name=twist_target_name, layer=twist_target_layer, group=twist_group, deform=False,
                                  lock_loc=False, lock_rot=True, lock_scale=True, type=None)
                    set_bone_shape(bone_name=twist_target_name, shape='twist_target', transform_bone=None,
                                   auto_scale_with_offset=None, manual_scale=target_shape_size, shape_bone=False,
                                   keep_rot_when_radius_check=False, use_bone_size=False,
                    ...
                    bone_settings(name=twist_target_line_name, layer=twist_target_layer, group=twist_group,
                                  deform=False, lock_loc=True, lock_rot=True, lock_scale=True, type=None)
                    set_bone_shape(bone_name=twist_target_line_name, shape='line', transform_bone=None,
                                   auto_scale_with_offset=None, manual_scale=1, shape_bone=False,
                                   keep_rot_when_radius_check=False, use_bone_size=True,
                                   shape_bone_parent_override=False)
                    ...
                    if end_affector != None:
                        ...
                        bpy.ops.constraint.childof_set_inverse(context_copy, constraint='Copy End Affector',
                                                               owner='BONE')
                ...
                for name in twist_bones:
                    bone_settings(name, layer=twist_layer, group=twist_group, deform=True, lock_loc=True,
                                  lock_rot=False, lock_scale=True, type='twist')
                    if lock_xz_rot == True:
                        bpy.ops.object.mode_set(mode='POSE')
            ...
            def twist_constraints(upper_or_lower, source_bone, target_bone, twist_bone_count, influences,
                                  source_bone_bend_axis, is_thigh):
            ...
            def chain_target(fk_chain, ik_chain, chain_target_distance, chain_target_size, target_name, shape,
                             use_copy_loc, copy_loc_target_bone, add_constraint_to_layer, module, prop_name):
                # target
                bpy.ops.object.mode_set(mode='EDIT')
                ...
                ebone.roll = 0
                bone_settings(name=target_name, layer=target_layer, group=target_group, deform=False, lock_loc=False,
                ...
                def create_servants(layer, source_bones):
                    ...
                    for name in source_bones:
                        ...
                        duplicate_bone(source_name=name, new_name=servant, parent_name=None, roll_override=False,
                                       length_override='HALF')
                        ebones = rig.data.edit_bones
                        ebone = ebones[servant]
                        ebone.roll = 0
                        ebone.parent = ebones[target_name]
                        translate_ebone_local(name=servant, vector=(0, 0, chain_target_distance))
                        bone_settings(name=servant, layer=layer, group=None, deform=False, lock_loc=True, lock_rot=True,
                                      lock_scale=True, type=None)
                ...
                def constraint_to_target(servants, constraint_bones):
                    ...
                    for index, name in enumerate(constraint_bones):
                        ...
                        prop_to_drive_constraint(name, bone_name=name, constraint_name=c.name,
                                                 prop_name=name + '_to_' + target_name, attribute='influence',
                                                 prop_min=0.0, prop_max=1.0, prop_default=0.0, description='',
                                                 expression='v1')
                ...
                if use_copy_loc:
                    ...
                    prop_to_drive_constraint(target_name, bone_name=target_name, constraint_name=c.name,
                                             prop_name='stick_to_' + copy_loc_target_bone, attribute='influence',
                                             prop_min=0.0, prop_max=1.0, prop_default=0.0, description='',
                                             expression='v1')

                set_bone_shape(bone_name=target_name, shape=shape, transform_bone=None, auto_scale_with_offset=None,
                               manual_scale=chest_target_size / 4, shape_bone=False, keep_rot_when_radius_check=False,
                               use_bone_size=False, shape_bone_parent_override=False)

                if add_constraint_to_layer:
                    prop_to_drive_layer(prop_bone='module_props__' + module, layer_index=target_layer,
                                        prop_name=prop_name, prop_min=0, prop_max=1, prop_default=0, description='',
                                        expression='v1')
            ...
            def get_radius(bone_name, number_of_checks, keep_rotation):
                ...
                hit_distances = []
                check_points = bone_radius_check_points (name=bone_name, number=number_of_checks, dist=distance)
            ...
            def set_bone_shape(bone_name, shape, transform_bone, auto_scale_with_offset, manual_scale, shape_bone,
                               keep_rot_when_radius_check, use_bone_size, shape_bone_parent_override):
                ...
                if shape_bone == 'CENTER' or shape_bone == 'LEAF':
                    ...
                    if shape_bone == 'LEAF':
                        ...
                        create_leaf_bone(source_bone=source_bone, name=shape_bone_name,
                                         length=get_distance(ebones[bone_name].head, ebones[bone_name].tail))
                    ...
                    translate_ebone_local (name=shape_bone_name, vector=v1)
                                                                               
                    hit_loc, hit_nor, hit_index, hit_dist = my_tree.ray_cast (rig.data.edit_bones[shape_bone_name].head, ebone.head, 10)
                    
                    translate_ebone_local (name=shape_bone_name, vector=v2)

                    if hit_dist != None:
                        translate_ebone_local (name=shape_bone_name, vector=v3)
            ...
            def bone_visibility(prop_bone, module, relevant_bones, ik_ctrl):
                ...
                for bone in fk_bones:
                    prop_to_drive_bone_attribute(prop_bone, bone_name=bone, bone_type='BONE',
                                                 prop_name='visible_fk_' + module, attribute='hide', prop_min=0,
                                                 prop_max=1, prop_default=1, description='', expression='1-v1')
                if ik_ctrl != None:
                    for bone in non_fk_bones:
                        prop_to_drive_bone_attribute(prop_bone, bone_name=bone, bone_type='BONE',
                                                     prop_name='visible_' + ik_ctrl + '_' + module, attribute='hide',
                                                     prop_min=0, prop_max=1, prop_default=1, description='',
                                                     expression='1-v1')
                if len(touch_bones) > 0:
                    for bone in touch_bones:
                        prop_to_drive_bone_attribute(prop_bone, bone_name=bone, bone_type='BONE',
                                                     prop_name='visible_touch_' + module, attribute='hide', prop_min=0,
                                                     prop_max=1, prop_default=1, description='', expression='1-v1')
            ...
            def prop_to_drive_constraint(prop_bone, bone_name, constraint_name, prop_name, attribute, prop_min,
                                         prop_max, prop_default, description, expression):
                # prop
                ...
                rig.pose.bones[prop_bone]["_RNA_UI"][prop_name] = {"min": prop_min, "max": prop_max,
                                                                   "soft_min": prop_min, "soft_max": prop_max,
                                                                   "description": description}
                # driver
                d = rig.driver_add(
                    'pose.bones["' + bone_name + '"].constraints["' + constraint_name + '"].' + attribute).driver
                v1 = d.variables.new()
                v1.name = 'v1'
                v1.type = 'SINGLE_PROP'
                t = v1.targets[0]
            ...
            def prop_to_drive_layer(prop_bone, layer_index, prop_name, prop_min, prop_max, prop_default, description,
                                    expression):
                # prop
                rig.pose.bones[prop_bone][prop_name] = prop_default
            ...
            def prop_to_drive_bone_attribute(prop_bone, bone_name, bone_type, prop_name, attribute, prop_min, prop_max,
                                             prop_default, description, expression):
                # prop
                rig.pose.bones[prop_bone][prop_name] = prop_default
            ...
            def prop_to_drive_pbone_attribute_with_array_index(prop_bone, bone_name, prop_name, attribute, array_index,
                                                               prop_min, prop_max, prop_default, description,
                                                               expression):
                # prop
                rig.pose.bones[prop_bone][prop_name] = prop_default
            ...
            def calculate_pole_target_location_2(b1, b2, b3, pole_target_distance, b2_bend_axis):
                ...
                duplicate_bone(source_name=b2, new_name=pole_bone, parent_name=None, roll_override=False,
            ...
            def three_bone_limb(module, n_b1, n_b2, n_b3, n_pole_target, pt_distance, parent_pt_to_ik_target, b2_bend_axis,
                                b2_bend_back, n_first_parent, fk_layer, ctrl_ik_layer, ik_group,
                                keep_last_bone_rot_when_radius_check, n_ik_parent, n_pole_parent, b3_shape_bone):

                n_pole_target = n_pole_target
                ...
                for name in bone_names:
                    bone_settings(name=name, layer=base_layer, group='base', deform=True, lock_loc=True, lock_rot=False,
                                  lock_scale=True, type='base')
                    n_relevant_bones.append(name)

                # _____________________________________________________________________________________________________

                # FK
                for index, name in enumerate(n_bones):
                    if index == 0:
                        n_parent = n_first_parent
                    else:
                        n_parent = fk_prefix + n_bones[index - 1]
                    duplicate_bone(source_name=name, new_name=fk_prefix + name, parent_name=n_parent,
                                   roll_override=False, length_override=None)

                    bone_settings(name=fk_prefix + name, layer=fk_layer, group=fk_group, deform=False, lock_loc=True,
                                  lock_rot=False, lock_scale=True, type='fk')
                    ...
                    if index == 2:
                        ...
                        keep_rot = False
                        shape = 'inner_circle'
#                    set_bone_shape(bone_name=fk_prefix + name, shape=shape, transform_bone=None,
#                                   auto_scale_with_offset=auto_bone_shape_scale_offset_limb, manual_scale=1,
#                                   shape_bone=shape_bone, keep_rot_when_radius_check=keep_rot, use_bone_size=False,
#                                   shape_bone_parent_override=False)

                    n_relevant_bones.append(fk_prefix + name)
                ...
                for axis in axis_locks_int:
                    prop_to_drive_pbone_attribute_with_array_index(fk_prefix + b2, bone_name=fk_prefix + b2,
                                                                   prop_name='limit_fk_' + module,
                                                                   attribute='lock_rotation', array_index=axis,
                                                                   prop_min=0, prop_max=1, prop_default=default,
                                                                   description='', expression='v1')
                ...
                prop_to_drive_constraint(fk_prefix + b2, bone_name=fk_prefix + b2, constraint_name='Limit rotation',
                                         prop_name='limit_fk_' + module, attribute='mute', prop_min=0, prop_max=1,
                                         prop_default=default, description='', expression='1-v1')
                ...
                for index, name in enumerate(bone_names):
                    ...
                    duplicate_bone(source_name=name, new_name=ik_prefix + name, parent_name=parent_name,
                                   roll_override=False, length_override=None)
                    ...
                    if index == 2:
                        ...
                        lock_loc = True
                    bone_settings(name=ik_prefix + name, layer=layer, group=ik_group, deform=False, lock_loc=lock_loc,
                                  lock_rot=False, lock_scale=True, type='ik')

                set_bone_shape(bone_name=ik_prefix + b3, shape='cube', transform_bone=None,
                               auto_scale_with_offset=auto_bone_shape_scale_offset_limb, manual_scale=1,
                               shape_bone=b3_shape_bone,
                               keep_rot_when_radius_check=keep_last_bone_rot_when_radius_check, use_bone_size=False,
                               shape_bone_parent_override=False)
                ...
                if create_ik_twist_bones == True:
                    ...
                    for name in twist_bones:
                        ...
                        duplicate_bone(source_name=name, new_name=ik_twist_bone, parent_name=ik_prefix + name,
                                       roll_override=False)
                        bone_settings(name=ik_twist_bone, layer=ctrl_ik_layer, group=ik_group, deform=False,
                                      lock_loc=True, lock_rot=False, lock_scale=True, type=None)
                ...
                bone_settings(name=ebone.name, layer=ctrl_ik_layer, group=ik_group, deform=False, lock_loc=False,
                              lock_rot=False, lock_scale=True, type='ik')
                set_bone_shape(bone_name=pole_target, shape='sphere', transform_bone=None, auto_scale_with_offset=None,
                ...
                bone_settings(name=pole_target_line, layer=ctrl_ik_layer, group=ik_group, deform=False, lock_loc=True,
                              lock_rot=True, lock_scale=True, type='ik')
                ...
                set_bone_shape(bone_name=pole_target_line, shape='line', transform_bone=None,
                               auto_scale_with_offset=None, manual_scale=1, shape_bone=None,
                               keep_rot_when_radius_check=False, use_bone_size=True, shape_bone_parent_override=False)
                ...
                duplicate_bone(source_name=pole_target, new_name=snap_target_pole,
                               parent_name=fk_prefix + bone_names[1], roll_override=False, length_override=False)
                bone_settings(name=snap_target_pole, layer=ctrl_ik_extra_layer, group=ik_group, deform=False,
                              lock_loc=True, lock_rot=True, lock_scale=True, type='ik')
                ...
                for name in bone_names:
                    prop_to_drive_constraint(prop_bone, bone_name=name, constraint_name='bind_to_fk_1',
                                             prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                             prop_default=0, description='0:fk, 1:ik, 2:bind',
                                             expression='1 - (v1 < 1)')
                    prop_to_drive_constraint(prop_bone, bone_name=name, constraint_name='bind_to_ik_1',
                                             prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                             prop_default=0, description='0:fk, 1:ik, 2:bind',
                ...
                while index_is_free is False:
                    for n in range(0, 1000):
                        ...
                        if prop_name_candidate not in pbone:
                            pbone[prop_name_candidate] = [fk_prefix + b1, fk_prefix + b2, fk_prefix + b3,
                                                          ik_prefix + b1, ik_prefix + b2, ik_prefix + b3, pole_target,
                                                          str(pole_target_distance), snap_target_pole, fk_prefix + b3,
                                                          ik_prefix + b3, 'None']
            ...
            def isolate_rotation(module, parent_bone, first_bone):
                ...
                bone_settings(name=first_intermeidate_bone, layer=fk_extra_layer, group=None, deform=False,
                              lock_loc=True, lock_rot=False, lock_scale=True, type=None)
                ...
                bone_settings(name=second_intermeidate_bone, layer=fk_extra_layer, group=None, deform=False,
                              lock_loc=True, lock_rot=False, lock_scale=True, type=None)
                ...
                prop_to_drive_constraint(first_bone, bone_name=second_intermeidate_bone,
                                         constraint_name='isolate_rot_1', prop_name='fixate_' + first_bone,
                                         attribute='influence', prop_min=0.0, prop_max=1.0, prop_default=0.0,
                                         description='', expression='1-v1')
            ...
            def finalize(visible_layers):
                ...
                prop_to_drive_layer(prop_bone, layer_index=base_layer, prop_name='visible_base_bones', prop_min=0,
                                    prop_max=1, prop_default=0, description='', expression='v1')
                prop_to_drive_layer(prop_bone, layer_index=twist_layer, prop_name='visible_twist_bones', prop_min=0,
                                    prop_max=1, prop_default=0, description='', expression='v1')
                prop_to_drive_layer(prop_bone, layer_index=spring_layer, prop_name='visible_spring_bones', prop_min=0,
                                    prop_max=1, prop_default=0, description='', expression='v1')
                prop_to_drive_layer(prop_bone, layer_index=face_layer, prop_name='visible_face_bones', prop_min=0,
                                    prop_max=1, prop_default=1, description='', expression='v1')
                prop_to_drive_layer(prop_bone, layer_index=face_extra_layer, prop_name='visible_extra_face_bones',
                                    prop_min=0, prop_max=1, prop_default=0, description='', expression='v1')
                prop_to_drive_layer(prop_bone, layer_index=root_layer, prop_name='visible_root_bone', prop_min=0,
                                    prop_max=1, prop_default=0, description='', expression='v1')
                prop_to_drive_layer(prop_bone, layer_index=ik_prop_layer, prop_name='visible_prop_bones', prop_min=0,
                                    prop_max=1, prop_default=0, description='', expression='v1')
                prop_to_drive_layer(prop_bone, layer_index=twist_target_layer, prop_name='visible_twist_targets',
                                    prop_min=0, prop_max=1, prop_default=0, description='', expression='v1')
...
#                    rig.data.layers[n] = False
            ...
            def root(size, root_extract_size):
                ...
                # format
                bone_settings(name, layer=root_layer, group=None, deform=False, lock_loc=False, lock_rot=False,
                              lock_scale=True, type=None)
                set_bone_shape('root', shape='master', transform_bone=None, auto_scale_with_offset=None, manual_scale=1,
                               shape_bone=None, keep_rot_when_radius_check=False, use_bone_size=False,
                               shape_bone_parent_override=False)
                ...
                # format
                bone_settings(name, layer=misc_layer, group=None, deform=False, lock_loc=True, lock_rot=True,
                              lock_scale=True, type=None)
            ...
            def ik_prop_bone(name, source_bone, parent):
                ...
                duplicate_bone(source_name=source_bone, new_name=name, parent_name=parent, roll_override=False,
                               length_override=False)
                bone_settings(name, layer=ik_prop_layer, group=ik_prop_group, deform=False, lock_loc=False,
                              lock_rot=False, lock_scale=True, type='ik_prop')
                set_bone_shape(name, shape='cube_outer', transform_bone=None,
                               auto_scale_with_offset=auto_bone_shape_scale_offset_limb, manual_scale=1,
                               shape_bone=None, keep_rot_when_radius_check=False, use_bone_size=False,
                               shape_bone_parent_override=False)
            ...
            def biped_torso(module, chain, first_parent):
                ...
                for index, name in enumerate(chain):
                    if index == 0:
                        ...
                        lock_loc = True
                    bone_settings(name=name, layer=base_layer, group='base', deform=True, lock_loc=lock_loc,
                ...
                for index, name in enumerate(chain):
                    ...
                    duplicate_bone(source_name=name, new_name=fk_prefix + name, parent_name=parent, roll_override=False,
                                   length_override=None)
                    bone_settings(name=fk_prefix + name, layer=fk_layer, group='fk', deform=False, lock_loc=lock_loc,
                                  lock_rot=False, lock_scale=True, type='fk')
                    set_bone_shape(bone_name=fk_prefix + name, shape=shape, transform_bone=None,
                                   auto_scale_with_offset=auto_bone_shape_scale_offset, manual_scale=1,
                                   shape_bone=shape_bone, keep_rot_when_radius_check=False, use_bone_size=False,
                                   shape_bone_parent_override=False)
                ...
                # format torso bone
                bone_settings(name=ctrl_torso, layer=ctrl_ik_layer, group=central_ik_group, deform=False,
                              lock_loc=False, lock_rot=False, lock_scale=True, type='ctrl')
                ...
                def spine_control(ctrl_name, length_multiplier, parent, is_on_main_layer, group):
                    ...
                    if is_on_main_layer == True:
                        ...
                        layer = ctrl_ik_extra_layer
                    bone_settings(name=ctrl_spine_name, layer=layer, group=group, deform=False, lock_loc=True,
                                  lock_rot=False, lock_scale=True, type='ctrl')
                ...
                prop_to_drive_constraint(ctrl_waist, bone_name=ctrl_waist_parent, constraint_name=c.name,
                                         prop_name='ctrl_waist_copy_chest', attribute='influence', prop_min=0.0,
                                         prop_max=1.0, prop_default=ctrl_waist__copy__ctrl_chest, description='',
                                         expression='v1')
                ...
                prop_to_drive_constraint(ctrl_waist, bone_name=ctrl_waist_parent, constraint_name=c.name,
                                         prop_name='ctrl_waist_copy_hips', attribute='influence', prop_min=0.0,
                                         prop_max=1.0, prop_default=ctrl_waist__copy__ctrl_hips, description='',
                                         expression='v1')
                ...
                for index, name in enumerate(chain):
                    ...
                    bone_settings(name=dist_bone_name, layer=ctrl_ik_extra_layer, group=None, deform=False,
                                  lock_loc=True, lock_rot=False, lock_scale=True, type=None)
                    ...
                    duplicate_bone(source_name=name, new_name=ik_bone_name, parent_name=dist_bone_name,
                                   roll_override=bone_roll, length_override=None)

                    # format ik bone
                    bone_settings(name=ik_bone_name, layer=ctrl_ik_extra_layer, group=None, deform=False, lock_loc=True,
                                  lock_rot=False, lock_scale=True, type=None)
                ...
                ebones[ik_hips_distributor_name].tail = ebones[ik_hips_distributor_name].head + Vector(
                    (0, 0, length / 3))
                # reset parents
                ...
                prop_to_drive_constraint(ctrl_chest, bone_name=pbone.name, constraint_name=c.name,
                                         prop_name='ctrl_spine_2_copy_chest', attribute='influence', prop_min=0.0,
                                         prop_max=1.0, prop_default=ik_spine_2__copy__ctrl_waist, description='',
                                         expression='v1')
                ...
                for index, name in enumerate(chain):
                    ...
                    duplicate_bone(source_name=name, new_name=ik_prefix + name, parent_name=parent_name,
                                   roll_override=False, length_override=None)
                    bone_settings(name=ik_prefix + name, layer=ctrl_ik_extra_layer, group=None, deform=False,
                                  lock_loc=True, lock_rot=False, lock_scale=True, type=None)
                ...
                for index, name in enumerate(chain):
                    if index > 0:
                        ...
                        duplicate_bone(source_name=name, new_name=rev_bone_name, parent_name=parent_name,
                                       roll_override=False, length_override=None)
                        mirror_bone_to_point(bone_name=rev_bone_name, point=ebones[ctrl_torso].head)
                        bone_settings(name=rev_bone_name, layer=ctrl_ik_extra_layer, group=None, deform=False,
                                      lock_loc=True, lock_rot=False, lock_scale=True, type=None)
                ...
                for index, name in enumerate(chain):
                    if index > 0:
                        ...
                        prop_to_drive_constraint(ctrl_torso, bone_name=bone_name, constraint_name=c.name,
                                                 prop_name='ctrl_spine_pivot_slide', attribute='influence',
                                                 prop_min=0.25, prop_max=1.0, prop_default=0.25, description='',
                                                 expression='(v1 - 0.25 * ' + str(index) + ') * 4')

                # control bone shapes
                set_bone_shape(bone_name=ctrl_hips, shape='inner_circle', transform_bone=ik_prefix + chain[1],
                               auto_scale_with_offset=auto_bone_shape_scale_offset, manual_scale=False, shape_bone=None,
                               keep_rot_when_radius_check=False, use_bone_size=False, shape_bone_parent_override=False)
                set_bone_shape(bone_name=ctrl_waist, shape='inner_circle', transform_bone=ik_prefix + chain[2],
                               auto_scale_with_offset=auto_bone_shape_scale_offset, manual_scale=False, shape_bone=None,
                               keep_rot_when_radius_check=False, use_bone_size=False, shape_bone_parent_override=False)
                set_bone_shape(bone_name=ctrl_chest, shape='inner_circle', transform_bone=ik_prefix + chain[3],
                               auto_scale_with_offset=auto_bone_shape_scale_offset, manual_scale=False, shape_bone=None,
                               keep_rot_when_radius_check=False, use_bone_size=False, shape_bone_parent_override=False)
                ...
                set_bone_shape(bone_name=ctrl_torso, shape='torso', transform_bone=None, auto_scale_with_offset=None,
                               manual_scale=scale, shape_bone=None, keep_rot_when_radius_check=True,
                               use_bone_size=False, shape_bone_parent_override=False)
                ...
                for index, name in enumerate(chain):
                    ...
                    for n in range(1, constraint_count + 1):
                        n = str(n)
                        prop_to_drive_constraint(prop_bone, bone_name=chain[index], constraint_name='bind_to_fk_' + n,
                                                 prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                                 prop_default=0, description='0:fk, 1:ctrl, 2:base',
                                                 expression='1 - (v1 < 1)')
                        prop_to_drive_constraint(prop_bone, bone_name=chain[index], constraint_name='bind_to_ik_' + n,
                                                 prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                                 prop_default=0, description='0:fk, 1:ctrl, 2:base',
                                                 expression='1 - (v1 > 0 and v1 < 2)')
            ...
            def short_neck(module, bone_name, distributor_parent, ik_rot_bone, ik_loc_bone):
                ...
                bone_settings(name=neck, layer=base_layer, group='base', deform=True, lock_loc=True, lock_rot=False,
                              lock_scale=True, type='base')
                ...
                bone_settings(name=name, layer=fk_layer, group=fk_group, deform=False, lock_loc=True, lock_rot=False,
                              lock_scale=True, type='fk')
                set_bone_shape(bone_name=name, shape='inner_circle', transform_bone=None,
                               auto_scale_with_offset=auto_bone_shape_scale_offset, manual_scale=1, shape_bone='CENTER',
                               keep_rot_when_radius_check=False, use_bone_size=False, shape_bone_parent_override=False)
                ...
                if use_twist == True:
                    twist_bones = create_twist_bones(source_bone=neck, count=1, upper_or_lower_limb='lower',
                                                     twist_target_distance=0, twist_target_parent='', lock_xz_rot=False,
                                                     end_affector=None, source_bone_bend_axis='')
                    ...
                    bone_settings(name=ebone.name, layer=misc_layer, group=None, deform=False, lock_loc=True,
                                  lock_rot=False, lock_scale=True, type=None)
                ...
                # bone settings
                set_bone_shape(distributor, shape='inner_circle', transform_bone=ik_prefix + neck,
                               auto_scale_with_offset=auto_bone_shape_scale_offset, manual_scale=1, shape_bone='CENTER',
                               keep_rot_when_radius_check=False, use_bone_size=False, shape_bone_parent_override=False)
                bone_settings(name=name, layer=ctrl_ik_extra_layer, group=None, deform=True, lock_loc=True,
                              lock_rot=False, lock_scale=True, type=None)
                bone_settings(name=distributor, layer=ctrl_ik_layer, group=central_ik_group, deform=False,
                              lock_loc=True, lock_rot=False, lock_scale=True, type=None)

                prop_to_drive_constraint(distributor, bone_name=distributor, constraint_name='inherit_rot',
                                         prop_name='fixate_ctrl_' + neck, attribute='influence', prop_min=0.0,
                                         prop_max=1.0, prop_default=fixate_ctrl_neck, description='', expression='1-v1')
                ...
                # BIND TO (0: FK, 1: IK, 2:BIND)
                prop_to_drive_constraint(prop_bone, bone_name=neck, constraint_name='bind_to_fk_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ctrl, 2:base', expression='1 - (v1 < 1)')
                prop_to_drive_constraint(prop_bone, bone_name=neck, constraint_name='bind_to_ik_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ctrl, 2:base',
                                         expression='1 - (v1 > 0 and v1 < 2)')
            ...
            def head(module, bone_name, ik_rot_bone, ik_loc_bone, distributor_parent):
                ...
                ebones[head].parent = ebones[parent]
                bone_settings(name=head, layer=base_layer, group='base', deform=True, lock_loc=True, lock_rot=False,
                              lock_scale=True, type='base')
                ...
                # bone settings
                set_bone_shape(distributor, shape='inner_circle', transform_bone=ik_prefix + head,
                               auto_scale_with_offset=auto_bone_shape_scale_offset, manual_scale=1, shape_bone='CENTER',
                               keep_rot_when_radius_check=False, use_bone_size=False, shape_bone_parent_override=False)
                bone_settings(name=distributor, layer=ctrl_ik_layer, group=central_ik_group, deform=False,
                              lock_loc=True, lock_rot=False, lock_scale=True, type=None)
                bone_settings(name=name, layer=ctrl_ik_extra_layer, group=None, deform=False, lock_loc=True,
                              lock_rot=False, lock_scale=True, type=None)

                prop_to_drive_constraint(distributor, bone_name=distributor, constraint_name='inherit_rot',
                                         prop_name='fixate_ctrl_' + head, attribute='influence', prop_min=0.0,
                                         prop_max=1.0, prop_default=fixate_ctrl_head, description='', expression='1-v1')
                ...
                # BIND TO (0: FK, 1: IK, 2:BIND)
                prop_to_drive_constraint(prop_bone, bone_name=head, constraint_name='bind_to_fk_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ctrl, 2:base', expression='1 - (v1 < 1)')
                prop_to_drive_constraint(prop_bone, bone_name=head, constraint_name='bind_to_ik_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ctrl, 2:base',
                                         expression='1 - (v1 > 0 and v1 < 2)')
            ...
            def spring_belly(module, waist_bones, loc_pelvis_front, loc_sternum_lower):
                ...
                if generate__spring_belly == True:
                    ...
                    belly_bones = subdivide_ebone (name='spring_belly', number=2, number_to_keep=2,
                                                   reverse_naming=False, prefix='', parent_all_to_source=False,
                                                   delete_source=True)
                    ...
                    for name in belly_bones:
                        # bone settings
                        bone_settings(name, layer=spring_layer, group=spring_group, deform=True, lock_loc=True,
                                      lock_rot=False, lock_scale=False, type='spring')
                    # deform
                    set_bone_deform('spring_belly', False)
                    # shape
                    set_bone_shape(bone_name='spring_belly', shape='sphere', transform_bone='belly',
                                   auto_scale_with_offset=None, manual_scale=target_shape_size, shape_bone='LEAF',
                                   keep_rot_when_radius_check=False, use_bone_size=False,
                                   shape_bone_parent_override=False)

                    belly_extra_bones = [loc_sternum_lower, loc_pelvis_front, 'target_belly', 'waist_mediator',
                                         'tracker_belly']
                    for name in belly_extra_bones:
                        # bone settings
                        bone_settings(name, layer=misc_layer, group=None, deform=False, lock_loc=True, lock_rot=False,
                                      lock_scale=False, type=None)
            ...
            def spring_chest(module, chest, shoulder):
                ...
                if generate__spring_chest == True:
                    ...
                    name = chest
                    bone_settings(name, layer=spring_layer, group=spring_group, deform=True, lock_loc=True,
                                  lock_rot=False, lock_scale=True, type='spring')
                    set_bone_shape(bone_name=chest, shape='sphere', transform_bone=None, auto_scale_with_offset=None,
                                   manual_scale=target_shape_size, shape_bone='LEAF', keep_rot_when_radius_check=False,
                                   use_bone_size=False, shape_bone_parent_override=False)
            ...
            def spring_bottom(module, source_bone, parent, side, source_bone_bend_axis):
                ...
                if generate__spring_bottom == True:
                    ...
                    rotate_ebone_local (name=bottom_raw, angle=angle, axis=axis)
                    ...
                    ebone.parent = ebones[parent]
                    # subdivide it
                    bottom_bones = subdivide_ebone (name=bottom_raw, number=2, number_to_keep=2,
                                                    reverse_naming=False, prefix='', parent_all_to_source=False,
                                                    delete_source=True)
                    bottom = 'bottom' + side
                    spring_bottom = 'spring_bottom' + side
                    ebones[bottom_bones[0]].name = spring_bottom
                    ebones[bottom_bones[1]].name = bottom

                    # thigh mediator
                    no_twist_name = create_no_twist_bone(source_bone=source_bone)

                    bone_settings(name=spring_bottom, layer=spring_layer, group=spring_group, deform=False,
                                  lock_loc=True, lock_rot=False, lock_scale=False, type='spring')
                    bone_settings(name=bottom, layer=spring_layer, group=spring_group, deform=True, lock_loc=True,
                                  lock_rot=False, lock_scale=False, type='spring')
                    set_bone_shape(bone_name=spring_bottom, shape='sphere', transform_bone=bottom,
                                   auto_scale_with_offset=None, manual_scale=target_shape_size, shape_bone='LEAF',
                                   keep_rot_when_radius_check=False, use_bone_size=False,
                                   shape_bone_parent_override=False)
            ...
            def biped_arm(module, n_chain, n_first_parent, n_pole_target, parent_pt_to_ik_target, forearm_bend_axis, forearm_bend_back, n_ik_parent, n_pole_parent, side):

                # chain length should be exactly 4
                n_shoulder = n_chain[0]
                n_upperarm = n_chain[1]
                n_forearm = n_chain[2]
                n_hand = n_chain[3]
                ...
                bpy.ops.object.mode_set(mode='EDIT')
                ebones = rig.data.edit_bones
                ebones[n_shoulder].parent = ebones[n_first_parent]
                ebones[n_upperarm].parent = ebones[n_shoulder]
                ebones[n_forearm].parent = ebones[n_upperarm]
                ebones[n_hand].parent = ebones[n_forearm]
                ...
                # SHOULDER BONE:
                # FK
                name = fk_prefix + n_shoulder
                duplicate_bone(source_name=n_shoulder, new_name=name, parent_name=n_first_parent, roll_override=False, length_override=None)
...
#                               use_bone_size=False, shape_bone_parent_override=False)
                ...
                bpy.ops.object.mode_set(mode='POSE')
                pbone = rig.pose.bones[n_shoulder]

                c = pbone.constraints.new('COPY_ROTATION')
                c.name = 'bind_to_fk_1'
                c.target = rig
                c.subtarget = fk_prefix + n_shoulder
                c.mute = True

                # filler bones (needed for GYAZ retargeter)
                bpy.ops.object.mode_set(mode='EDIT')
                n_filler = name='fk_filler_' + n_shoulder
                ebone = rig.data.edit_bones.new(n_filler)
                ebones = rig.data.edit_bones
                ebone.head = ebones[n_first_parent].head
                ebone.tail = ebones[n_shoulder].head
                ebone.roll = 0
                ebone.parent = ebones[n_first_parent]
                set_bone_only_layer(bone_name=n_filler, layer_index=fk_extra_layer)
...
#                               use_bone_size=False, shape_bone_parent_override=False)
                ...
                c.name = 'bind_to_ik_1'
                c.target = rig
                c.subtarget = ik_prefix + n_shoulder
                c.mute = True

                # BIND TO (0: FK, 1: IK, 2:BIND)
                prop_to_drive_constraint(n_prop_bone, bone_name=n_shoulder, constraint_name='bind_to_fk_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ik, 2:base', expression='1 - (v1 < 1)'
                                         )
                prop_to_drive_constraint(n_prop_bone, bone_name=n_shoulder, constraint_name='bind_to_ik_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ik, 2:base',
                                         expression='1 - (v1 > 0 and v1 < 2)'
                                         )

                bone_visibility(n_prop_bone, module, n_relevant_bones, ik_ctrl='ik')

                # SNAP INFO
                bpy.ops.object.mode_set(mode='POSE')
                pbone = rig.pose.bones[n_prop_bone]
                pbone['snapinfo_singlebone_0'] = [fk_prefix + n_shoulder, ik_prefix + n_shoulder]

                # three bone limb set-up
                three_bone_limb(module=module, b1=n_upperarm, b2=n_forearm, b3=n_hand, pole_target=n_pole_target,
                                pt_distance=pole_target_distance, parent_pt_to_ik_target=parent_pt_to_ik_target,
                                b2_bend_axis=forearm_bend_axis, b2_bend_back=forearm_bend_back, first_parent=n_shoulder,
                                fk_layer=fk_layer, ctrl_ik_layer=ctrl_ik_layer, ik_group=ik_group,
                                keep_last_bone_rot_when_radius_check=False, ik_parent=n_ik_parent,
                                pole_parent=n_pole_parent, b3_shape_bone=None)

                isolate_rotation(module=module, parent_bone=fk_prefix + n_shoulder, first_bone=fk_prefix + n_upperarm)

                # set module name on relevant bones (used by the 'N-panel' interface)
                set_module_on_relevant_bones(n_relevant_bones, module=module)
            ...
            def biped_leg(module, chain, pole_target, parent_pt_to_ik_target, shin_bend_axis, shin_bend_back, ik_parent,
                          foot_toes_bend_axis, pole_parent, side):
                ...
                # BIND TO (0: FK, 1: IK, 2:BIND)
                prop_to_drive_constraint(prop_bone, bone_name=chain[3], constraint_name='bind_to_fk_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ik, 2:base', expression='1 - (v1 < 1)')
                prop_to_drive_constraint(prop_bone, bone_name=chain[3], constraint_name='bind_to_ik_1',
                                         prop_name='switch_' + module, attribute='mute', prop_min=0, prop_max=2,
                                         prop_default=0, description='0:fk, 1:ik, 2:base',
                                         expression='1 - (v1 > 0 and v1 < 2)')
            ...
            def twist_bones(source_bone, number, upper_or_lower_limb, twist_target_parent, twist_distribution,
                            end_affector, source_bone_bend_axis, is_thigh):
                if number > 0:
                    count = 3
                    twist_bones = create_twist_bones(source_bone=source_bone, count=count,
                                                     upper_or_lower_limb=upper_or_lower_limb,
                                                     twist_target_distance=pole_target_distance,
                                                     twist_target_parent=twist_target_parent, lock_xz_rot=True,
                                                     end_affector=end_affector,
                                                     source_bone_bend_axis=source_bone_bend_axis)

                    twist_influences = twist_distribution[number - 1]

                    twist_constraints(upper_or_lower=upper_or_lower_limb, source_bone=source_bone,
                                      target_bone='twist_target_' + source_bone, twist_bone_count=number,
                                      influences=twist_influences, source_bone_bend_axis=source_bone_bend_axis,
                                      is_thigh=is_thigh)
            ...
            def fingers(module, finger_names, side, bend_axis):
                ...
                if generate__fingers == True:
                    ...
                    for finger in finger_names:
                        for n in range(1, 4):
                            name = finger + '_' + str(n) + side
                            bone_settings(name=name, layer=base_layer, group='base', deform=True, lock_loc=True,
                                          lock_rot=False, lock_scale=True, type='base')
                            relevant_bones.append(name)
                    ...
                    def create_finger_bones(prefix, set_shape, layer, group, constraint_name, type):
                        ...
                        for finger in finger_names:
                            for n in range(1, 4):
                                name = prefix + finger + '_' + str(n) + side
                                bone_settings(name=name, layer=layer, group=group, deform=False, lock_loc=True,
                                              lock_rot=False, lock_scale=True, type=type)
                                if set_shape == True:
                                    set_bone_shape(bone_name=name, shape='sphere', transform_bone=None,
                                                   auto_scale_with_offset=None, manual_scale=finger_shape_size,
                                                   shape_bone='-X', keep_rot_when_radius_check=False,
                                                   use_bone_size=False, shape_bone_parent_override=False)
                                relevant_bones.append(name)
                    ...
                    create_finger_bones(prefix=fk_prefix, set_shape=True, layer=fk_layer, group=fk_group,
                                        constraint_name='bind_to_fk_1', type='fk')

                    # IK
                    create_finger_bones(prefix=ik_prefix, set_shape=False, layer=ctrl_ik_extra_layer, group=None,
                                        constraint_name='bind_to_ik_1', type=None)
                    ...
                    for index, finger in enumerate(finger_names):
                        ...
                        ebone.parent = ebones[first_parent]
                        bone_settings(name=ctrl_name, layer=ctrl_ik_layer, group=ik_group, deform=False, lock_loc=True,
                                      lock_rot=False, lock_scale=False, type='ctrl')
                        set_bone_shape(bone_name=ctrl_name, shape='cube', transform_bone=None,
                                       auto_scale_with_offset=None, manual_scale=finger_shape_size, shape_bone='-X',
                                       keep_rot_when_radius_check=False, use_bone_size=False,
                                       shape_bone_parent_override=False)
                    ...
                    for index, finger in enumerate(finger_names):
                        ...
                        if index == 0:
                            ...
                            add_c(pbone, ctrl_finger_scale__to_finger_2_3_bend_fwd__scale,
                                  ctrl_finger_scale__to_thumb_2_bend_fwd__rot,
                                  ctrl_finger_scale__to_finger_2_3_bend_bwd__scale,
                                  ctrl_finger_scale__to_thumb_2_bend_bwd__rot)
                            ...
                            for n in range(2, 4):
                                ...
                                add_c(pbone, ctrl_finger_scale__to_finger_2_3_bend_fwd__scale,
                                      ctrl_finger_scale__to_finger_2_3_bend_fwd__rot,
                                      ctrl_finger_scale__to_finger_2_3_bend_bwd__scale,
                                      ctrl_finger_scale__to_finger_2_3_bend_bwd__rot)
                    ...
                    for finger in finger_names:
                        for n in range(1, 4):
                            name = finger + '_' + str(n) + side
                            prop_to_drive_constraint(prop_bone, bone_name=name, constraint_name='bind_to_fk_1',
                                                     prop_name='switch_' + module, attribute='mute', prop_min=0,
                                                     prop_max=2, prop_default=0, description='0:fk, 1:ctrl, 2:bind',
                                                     expression='1 - (v1 < 1)')
                            prop_to_drive_constraint(prop_bone, bone_name=name, constraint_name='bind_to_ik_1',
                                                     prop_name='switch_' + module, attribute='mute', prop_min=0,
                                                     prop_max=2, prop_default=0, description='0:fk, 1:ctrl, 2:bind',
            ...
            def touch_bone(module, source_bone, ik_bone, side, shape_bone):
                ...
                duplicate_bone(source_name=source_bone, new_name=name, parent_name=None, roll_override=False,
                               length_override=False)
                bone_settings(name, layer=touch_layer, group=group, deform=False, lock_loc=False, lock_rot=False,
                              lock_scale=True, type='touch')

                bpy.ops.object.mode_set(mode='POSE')
                pbones = rig.pose.bones

                scale = pbones[ik_bone].custom_shape_scale
                set_bone_shape(bone_name=name, shape='twist_target', transform_bone=None, auto_scale_with_offset=None,
                               manual_scale=scale, shape_bone=shape_bone, keep_rot_when_radius_check=False,
                               use_bone_size=False, shape_bone_parent_override=False)
                ...
                prop_to_drive_constraint(ik_bone, bone_name=ik_bone, constraint_name='Copy Touch Bone',
                                         prop_name='touch_active_' + module, attribute='influence', prop_min=0.0,
                                         prop_max=1.0, prop_default=0.0, description='', expression='v1')
            ...
            def face_base(module, use_jaw, parent):
                ...
                if generate__face_eyes == True:
                    ...
                    duplicate_bone(source_name='head', new_name='face_parent', parent_name=parent, roll_override=False,
                                   length_override='HALF')
                    ...
                    for name, bone_parent, use_deform, lock_loc, lock_rot, bone_shape in bone_table:
                        ...
                        if look in name:
                            ...
                            group = face_group
                        bone_settings(name, layer=face_layer, group=group, deform=use_deform, lock_loc=lock_loc,
                                      lock_rot=lock_rot, lock_scale=True, type='face')
                        ...
                        if bone_shape != None:
                            ...
                            set_bone_shape(bone_name=name, shape=bone_shape, transform_bone=transform_bone,
                                           auto_scale_with_offset=None, manual_scale=scale, shape_bone=shape_bone,
                                           keep_rot_when_radius_check=keep_rot_when_radius_check, use_bone_size=False,
                                           shape_bone_parent_override=shape_bone_parent_override)
                    ...
                    if use_jaw == True:
                        ...
                        prop_to_drive_constraint('jaw', bone_name='teeth_lower', constraint_name='Copy Jaw',
                                                 prop_name='teeth_lower_follow_jaw', attribute='influence',
                                                 prop_min=0.0, prop_max=1.0, prop_default=teeth_lower_copy_jaw_rot,
                                                 description='', expression='v1')
                    ...
                    for side in sides:
                        ...
                        prop_to_drive_constraint(look, bone_name='eye' + side, constraint_name='Eye Target',
                                                 prop_name='active_look_target', attribute='mute', prop_min=0,
                                                 prop_max=1, prop_default=0, description='', expression='1-v1')
                        prop_to_drive_constraint('upperlid' + side, bone_name='upperlid' + side,
                                                 constraint_name='Copy Eye', prop_name='upperlid_follow_eye',
                                                 attribute='influence', prop_min=0.0, prop_max=1.0,
                                                 prop_default=upperlid_copy_eye_rot, description='', expression='v1')
                        prop_to_drive_constraint('lowerlid' + side, bone_name='lowerlid' + side,
                                                 constraint_name='Copy Eye', prop_name='lowerlid_follow_eye',
                                                 attribute='influence', prop_min=0.0, prop_max=1.0,
                                                 prop_default=lowerlid_copy_eye_rot, description='', expression='v1')
            ...
            def face_detail(module):
                ...
                if generate__face_detail == True:
                    ...
                    duplicate_bone(source_name='jaw', new_name='lowerlip', parent_name=parent, roll_override=False,
                                   length_override='HALF')
                    ...
                    for name, bone_parent, use_deform, lock_loc, lock_rot, bone_shape in bone_table:
                        ...
                        ebones[name].parent = ebones[bone_parent]
                        ebones[name].roll = 0
                        bone_settings(name, layer=face_layer, group=face_group, deform=use_deform, lock_loc=lock_loc,
                                      lock_rot=lock_rot, lock_scale=True, type='face')

                        if bone_shape != None:
                            set_bone_shape(bone_name=name, shape=bone_shape, transform_bone=None,
                                           auto_scale_with_offset=None, manual_scale=face_shape_size, shape_bone='-X',
                                           keep_rot_when_radius_check=False, use_bone_size=False,
                                           shape_bone_parent_override=False)
                    ...
                    prop_to_drive_constraint('jaw', bone_name='lowerlip', constraint_name='Copy Jaw',
                                             prop_name='lowerlip_follow_jaw', attribute='influence', prop_min=0.0,
                                             prop_max=1.0, prop_default=lowerlip_copy_jaw_rot, description='',
                                             expression='v1')
...
#            short_neck(module='spine', bone_name='neck', ik_rot_bone='ctrl_chest', ik_loc_bone=ik_prefix + 'spine_3',
#                       distributor_parent='ctrl_torso')
#            head(module='spine', bone_name='head', ik_rot_bone='ctrl_neck', ik_loc_bone=ik_prefix + 'neck',
#                 distributor_parent='ctrl_torso')
#            chain_target(fk_chain=[fk_prefix + 'neck', fk_prefix + 'head'], ik_chain=['ctrl_neck', 'ctrl_head'],
#                         chain_target_distance=chest_target_distance, chain_target_size=chest_target_size,
#                         target_name='target_head', shape='sphere', use_copy_loc=False, copy_loc_target_bone='',
#                         add_constraint_to_layer=True, module='spine', prop_name='visible_spine_targets')
#            chain_target(
#                fk_chain=[fk_prefix + 'hips', fk_prefix + 'spine_1', fk_prefix + 'spine_2', fk_prefix + 'spine_3'],
#                ik_chain=['ctrl_chest'], chain_target_distance=chest_target_distance,
#                chain_target_size=chest_target_size, target_name='target_chest', shape='cube', use_copy_loc=True,
#                copy_loc_target_bone='target_head', add_constraint_to_layer=False, module='', prop_name='')
...
#            spring_belly(module='spring', waist_bones=['spine_1', 'spine_2'], loc_pelvis_front='loc_pelvis_front',
#                         loc_sternum_lower='loc_sternum_lower')

            for side in sides:
                biped_arm(module='arm' + side, n_chain=['shoulder' + side, 'upperarm' + side, 'forearm' + side, 'hand' + side],
                          n_first_parent='spine_3', n_pole_target='elbow' + side, parent_pt_to_ik_target=False, forearm_bend_axis='X',
                          forearm_bend_back=30, n_ik_parent='ik_hand_prop', n_pole_parent='root_extract', side=side)
#                biped_leg(module='leg' + side, chain=['thigh' + side, 'shin' + side, 'foot' + side, 'toes' + side],
#                          pole_target='knee' + side, parent_pt_to_ik_target=True, shin_bend_axis='-X', shin_bend_back=0,
#                          ik_parent='root_extract', foot_toes_bend_axis='-X', pole_parent='root_extract', side=side)
#                fingers(module='fingers' + side, finger_names=['thumb', 'pointer', 'middle', 'ring', 'pinky'],
#                        side=side, bend_axis='-X')
#                touch_bone(module='arm' + side, source_bone='hand' + side, ik_bone=ik_prefix + 'hand' + side, side=side,
#                           shape_bone=None)
#                touch_bone(module='leg' + side, source_bone='foot' + side, ik_bone=ik_prefix + 'main_foot' + side,
#                           side=side, shape_bone='FRONT')
#                twist_bones(source_bone='upperarm' + side, number=generate__twist_upperarm_count,
#                            upper_or_lower_limb='upper', twist_target_parent='shoulder' + side,
#                            twist_distribution=upperarm_twists, end_affector=None, source_bone_bend_axis='-X',
#                            is_thigh=False)
#                twist_bones(source_bone='forearm' + side, number=generate__twist_forearm_count,
#                            upper_or_lower_limb='lower', twist_target_parent='', twist_distribution=forearm_twists,
#                            end_affector='hand' + side, source_bone_bend_axis='-X', is_thigh=False)
#                twist_bones(source_bone='thigh' + side, number=generate__twist_thigh_count, upper_or_lower_limb='upper',
#                            twist_target_parent='hips', twist_distribution=thigh_twists, end_affector=None,
#                            source_bone_bend_axis='-X', is_thigh=True)
#                twist_bones(source_bone='shin' + side, number=generate__twist_shin_count, upper_or_lower_limb='lower',
#                            twist_target_parent='', twist_distribution=shin_twists, end_affector='foot' + side,
#                            source_bone_bend_axis='-X', is_thigh=False)
#                spring_chest(module='spring', chest='spring_chest' + side, shoulder='shoulder' + side)
#                spring_bottom(module='spring', source_bone='thigh' + side, parent='hips', side=side,
